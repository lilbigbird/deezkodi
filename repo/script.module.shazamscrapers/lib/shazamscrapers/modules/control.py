...
addonObject = addon('script.module.shazamscrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from shazamscrapers.modules import libtools
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.shazam.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.shazam.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.shazam.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.shazam.artwork)')
...
# def metaFile():
    # if condVisibility('System.HasAddon(script.shazam.metadata'):
        # return os.path.join(xbmcaddon.Addon('script.shazam.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
