...
DEBUGPREFIX = '[ shazamscrapers {0} | DEBUG ]'.format(version)
INFOPREFIX = '[ shazamscrapers | INFO ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'shazam.log')
debug_enabled = control.addon('plugin.video.shazam').getSetting('addon.debug')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('shazamscrapers Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
