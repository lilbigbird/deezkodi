...
def adv_gen_window(url):
    ...
    class adv_gen_window(pyxbmct.AddonDialogWindow):
        ...
        def set_info_controls(self):
            ...
            url='http://api.themoviedb.org/3/genre/%s/list?api_key=6ab389aca8129c1c2192d54c81614f46&language=%s&page=1'%(self.type,lang)
            x=get_html(url).json()
            self.all_g=[]
...
def get_html_g():
    try:
        url_g='https://api.themoviedb.org/3/genre/tv/list?api_key=6ab389aca8129c1c2192d54c81614f46&language='+lang
        html_g_tv=get_html(url_g).json()
         
   
        url_g='https://api.themoviedb.org/3/genre/movie/list?api_key=6ab389aca8129c1c2192d54c81614f46&language='+lang
           ...
           for data in result_html:
             ...
             if 'original_title' in data:
               ...
               if data['original_language']!='en':
                ...
                html2=get_html('http://api.themoviedb.org/3/movie/%s?api_key=6ab389aca8129c1c2192d54c81614f46'%id).json()
               ...
               if data['original_language']!='en':
                    ...
                    html2=get_html('http://api.themoviedb.org/3/tv/%s?api_key=6ab389aca8129c1c2192d54c81614f46'%id).json()
                    if 'name' in html2:
...
def get_seasons(name,url,iconimage,fanart,description,data,original_title,id,heb_name):
   ...
   t = TVDB()
   if 'tvdb' not in id:
       url=domain_s+'api.themoviedb.org/3/tv/%s?api_key=6ab389aca8129c1c2192d54c81614f46&language=%s&append_to_response=external_ids'%(id,lang)
...
def get_episode_data(id,season,episode,yjump=True,o_name=' '):
    o_season=season
    o_episode=episode
    url='http://api.themoviedb.org/3/tv/%s/season/%s/episode/%s?api_key=6ab389aca8129c1c2192d54c81614f46&language=%s&append_to_response=external_ids'%(id,season,episode,lang)
    ...
    if yjump:
      ...
      if 'status_code' in html or ('error_code' in html and html['error_code']==404):
        log.warning('In::')
        url='http://api.themoviedb.org/3/tv/%s/season/%s/episode/%s?api_key=6ab389aca8129c1c2192d54c81614f46&language=%s&append_to_response=external_ids'%(id,str(int(season)+1),'1',lang)
        html=get_html(url).json()
        episode='1'
...
def get_episode(name,url,iconimage,fanart,description,data,original_title,id,season,tvdb_id,show_original_year,heb_name):
   ...
   url=domain_s+'api.themoviedb.org/3/tv/%s/season/%s?api_key=6ab389aca8129c1c2192d54c81614f46&language=%s&append_to_response=external_ids'%(id,season,lang)
   tmdbKey = '6ab389aca8129c1c2192d54c81614f46'
   html=get_html(url).json()
   #tmdb data
   ...
   if 'episodes'  in html:
      if len(html['episodes'])>0:
       ...
       if 'name' not in html['episodes'][0] or html['episodes'][0]['name']=='':
         url=domain_s+'api.themoviedb.org/3/tv/%s/season/%s?api_key=6ab389aca8129c1c2192d54c81614f46&language=eng'%(id,season)
