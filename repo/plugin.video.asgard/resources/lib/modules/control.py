...
def artPath():
    ...
    if theme in ['-', '']:
        ...
        return os.path.join(
            xbmcaddon.Addon('script.shazam.artwork').getAddonInfo('path'),
            'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility(
        'System.HasAddon(script.shazam.artwork)') else setting('appearance.alt').lower()
    return appearance
...
def metaFile():
    if condVisibility('System.HasAddon(script.shazam.metadata)'):
        return os.path.join(
            xbmcaddon.Addon('script.shazam.metadata').getAddonInfo('path'),
