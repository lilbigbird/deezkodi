...
#TC 2/01/19 started
        ...
        if control.addonInfo('id') == 'plugin.video.shazam':
            try:
                ...
                if quality == 'AUTO':
                    ...
                    control.window.clearProperty("plugin.video.shazam.container.items")
                    control.window.setProperty("plugin.video.shazam.container.items", json.dumps(items))

                    control.window.clearProperty("plugin.video.shazam.container.meta")
                    control.window.setProperty("plugin.video.shazam.container.meta", meta)
                    control.window.clearProperty(self.itemProperty)
                    control.window.setProperty(self.itemProperty, json.dumps(items))
    ...
    def sourcesFilter(self):
        ...
        sortshazam = control.setting('torrent.sort.1')
        if sortshazam == '':
            sortshazam = 'false'
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.shazam.container.items'

        self.metaProperty = 'plugin.video.shazam.container.meta'
