...
class cTMDb:
    ...
    CACHE = 'special://home/userdata/addon_data/plugin.video.mando/video_cache.db'
    ...
    def _format(self, meta, name):
        ...
        if 'credits' in meta and meta['credits']:
            ...
            if len(casts) > 0:
                #licast = []
                if 'shazam' in listCredits:
                    shazams = listCredits['shazam']
                if len(shazams)>0:
                    _meta['credits'] = "{u'cast': " + str(casts) + ", u'shazam': "+str(shazams) + "}"
                else:
...
#                 _meta['credits'] = 'u\'cast\': ' + str(casts) + ''
            ...
            if len(shazams) > 0:
                for shazam in shazams:
                    if shazam['job'] == 'Director':
                        _meta['director'] = shazam['name']
                    elif shazam['department'] == 'Writing':
                        if _meta['writer'] != '':
                            _meta['writer'] += ' / '
                        _meta['writer'] += '%s (%s)' % (shazam['job'], shazam['name'])
                    elif shazam['department'] == 'Production' and 'Producer' in shazam['job']:
                        if _meta['writer'] != '':
                            _meta['writer'] += ' / '
                        _meta['writer'] += '%s (%s)' % (shazam['job'], shazam['name'])
